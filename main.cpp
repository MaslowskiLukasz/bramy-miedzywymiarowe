...
void *RecvMsg(void *arg) {
	while(true) {
		...
		printf("%c[%dm %d : %d dostał wiadomosc typu: %d od %d\n", 0x1B, RED, rank, my_clock, status.MPI_TAG, status.MPI_SOURCE);
		...
		switch(status.MPI_TAG) {
			case MSG_REQ: {
				...
				if(medium_id != medium_req) {
					printf(" %d : %d wysyla pozowlenie do %d (medium != medium)\n", rank, my_clock, sender_id);
			...
			case MSG_PERMISSION: {
				printf("%c[%dm %d : %d dostal zgode od turysty %d -> %d / %d\n", 0x1B, PINK, rank, my_clock, sender_id, PROCESS_NUMBER - ack_counter, PROCESS_NUMBER - 1);
				ack_counter--;
				if(ack_counter == 0) {
